...
| 2026-08-04 | 2 | Claude | A rule believed enforced was not: "no UI string is hardcoded" was checked by comparing catalogues, which cannot see a string that never reads one. Errors now carry a key, and a second test checks the call sites. Twitch's own error text is quoted inside a translated sentence rather than shown bare. See #76. |
